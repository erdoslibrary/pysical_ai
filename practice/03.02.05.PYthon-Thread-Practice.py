...
def producer(q, stop_event):
    ...
    while not stop_event.is_set():
        ...
        q.put(sensor_value)
        print(f"[Producer] 생성: {sensor_value}")
        time.sleep(0.1)
...
def consumer(q, stop_event):
    ...
    while not stop_event.is_set() or not q.empty():
        try:
            sensor_value = q.get(timeout=0.1)
            print(f"[Consumer] 처리: {sensor_value:.2f} → 정상" if sensor_value < 80 else f"[Consumer] 처리: {sensor_value:.2f} → 경고!")
        except queue.Empty:
            continue
